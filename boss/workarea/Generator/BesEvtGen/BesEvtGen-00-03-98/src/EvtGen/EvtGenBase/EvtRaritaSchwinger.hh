...
EvtRaritaSchwinger conj(const EvtRaritaSchwinger& u2);  //pangcy add 
...
class EvtRaritaSchwinger{
  ...
  friend EvtRaritaSchwinger conj(const EvtRaritaSchwinger& u2);//pangcy add  
...
//  inline const EvtComplex& get(int i, int j) const;  //pangcy add

  void applyRotateEuler(double alpha,double beta,double gamma);
  void applyBoostTo(const EvtVector4R p4);
  void applyBoostTo(const EvtVector3R boost);
  EvtRaritaSchwinger conj() const;   //pangcy add
