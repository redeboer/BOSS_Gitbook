...
*  #[ data:
    ...
    1	    continue
	    call ffopen(ifile,'phokhara_9.1.fferr',ier)
  ...
  100	    continue
	    print '(a)',
     +		'fferr: Cannot open phokhara_9.1.fferr with error texts'
  110	    continue
	    close(ifile)
	endif
*  #] data:
...
*  #[ data:
	...
	if ( init.eq.0 .and. nerr.ne.999 ) then
     ...
     +  	'ffwarn:  warning:   illegal value for ierr'
		xlosti(i) = 0
    1	    continue
            call ffopen(ifile,'phokhara_9.1.ffwarn',ier)
	    if ( ier.ne.0 ) goto 100
	    rewind(ifile)
	    read(ifile,'(a)')warn1
	    read(ifile,'(a)')warn1
  ...
  100	    continue
	    print '(a)',
     +'ffwarn: Cannot open phokhara_9.1.ffwarn with warning texts'
  110	    continue
	    close(ifile)
	    laseve = -1
	    imem = 1
