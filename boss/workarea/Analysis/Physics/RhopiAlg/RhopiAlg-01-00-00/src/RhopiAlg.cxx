 ...
 * @details Here, you should declare properties: give them a name, assign a parameter (data member of `RhopiAlg`), and if required a documentation string. Note that you should define the paramters themselves in the header (RhopiAlg/RhopiAlg.h) and that you should assign the values in `share/jopOptions_Rhopi.txt`.
 */
	...
	// * NTuple: Rho fit branch (fit5c) * //
		if(fElectronst5C) {
			...
			if ( nt5 ) fTupleFit5C = nt5;
				...
				if(fTupleFit5C) {
					fTupleFit5C->addItem("chi2",  fChi2);  // chi squared of the Kalman kinematic fit
					fTupleFit5C->addItem("mrho0", fMrho0); // inv. mass pi^+ pi^- (rho^0)
					fTupleFit5C->addItem("mrhop", fMrhop); // inv. mass pi^0 pi^+ (rho^+)
					fTupleFit5C->addItem("mrhom", fMrhom); // inv. mass pi^0 pi^- (rho^-)
				}
				else {
