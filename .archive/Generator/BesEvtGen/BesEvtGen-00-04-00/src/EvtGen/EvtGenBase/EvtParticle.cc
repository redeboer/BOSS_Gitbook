...
#include "EvtGenBase/EvtRaritaSchwingerParticle.hh"  //Pingrg  2007.3.15
...
void EvtParticle::decay(){
  ...
  if ( _ndaug==1 &&  (getId()==BS0||getId()==BSB||getId()==BD0||getId()==BDB) ) 
    {getDaug(0)->decay();
      std::cout<<"if ( _ndaug==1 &&  (getId()==BS0||getId()==BSB||getId()==BD0||getId()==BDB) )"<<endl;
    }
...
void EvtParticle::generateMassTree() {
  ...
  while (massProb<ranNum) {
   ...
   // report(INFO,"EvtGen") << "end of iter " << massProb <<endl;
    ...
    if ( counter > 10000 ) {
      if ( counter == 10001 ) {
	report(INFO,"EvtGen") << "Too many iterations to determine the mass tree. Parent mass= "<< p->mass() << _p<<" " << massProb <<endl;
	p->printTree();
	...
	if ( massProb>0. ) {
	  ...
	  std::cout<<EvtPDL::name(p->getId())<<": Parent mass "<<p->getP4().mass()<<" with momentum "<<p->getP4()<<std::endl;
	  if(EvtGlobalSet::ConExcPythia){EvtGlobalSet::ConExcPythia=0;return;}else{abort();}
	  //assert(0);
	}
      }
    }
...
std::string EvtParticle::writeTreeRec(std::string resonance) const {  //pingrg, Jun. 6, 2008
  ...
  if (resonance ==  EvtPDL::name(_id).c_str() && _ndaug!=0) {
    ...
    for(int i=0;i<_ndaug;i++){
      _daug[i]->writeTreeRec(resonance);
    }
    std::cout<<retval;
  return retval;
}
...
void EvtParticle::dumpTreeRec(int level,int dj) const {  //pingrg, Mar. 25,2008
  ...
  if (_ndaug!=0) {
    ...
    for(i=0;i<_ndaug;i++){
       _daug[i]->dumpTreeRec(newlevel,i);
    }
  }
}
