...
EvtDecayBase* EvtDecayTable::gettheDecay(EvtId parent, int imode){
  int ipar=parent.getAlias();
  EvtDecayBase* thedecaymodel=decaytable[ipar].getDecay(imode).getDecayModel();
  return thedecaymodel;
} 
...
void EvtDecayTable::readDecayFile(const std::string dec_name){
  ...
  do{
    ...
    }else if ( token=="AddFactorPn") {
      std::string pname;
      pname=parser.getToken(itoken++);
      double factor=atof(parser.getToken(itoken++).c_str());
      EvtId thisPart = EvtPDL::getId(pname);
      EvtPDL::addFactorPn(thisPart,factor);
      report(DEBUG,"EvtGen") <<"Include momentum factor Pn= "<<factor <<"  for " << EvtPDL::name(thisPart).c_str() <<endl;
	...
	::abort();
      ...
      if (decaytable[newipar.getAlias()].getNMode()!=0) {
	report(DEBUG,"EvtGen") <<"Redefining decay of "
			       <<newname<<endl;
	decaytable[newipar.getAlias()].removeDecay();
      }
      decaytable[newipar.getAlias()] = decaytable[oldipar.getAlias()];
    }
    ...
    // Peter Onyisi, March 2008
	  ...
	  temp_fcn_new->saveDecayInfo(ipar, n_daugh,
                                      ...
                                      temp_fcn_new_model,
                                      0.);
	  decaytable[ipar.getAlias()].removeMode(temp_fcn_new);
        }
      } while (token != "Enddecay");
      itoken++;
    }
...
int  EvtDecayTable::inChannelList(EvtId parent, int ndaug, EvtId *daugs){
   ...
   int i,j,k;
   //  std::cout<<"=============MAX_DAUG = "<<MAX_DAUG<<endl;
	 ...
	 for(j=0;j<nd;j++){
	   for(k=0;k<ndaug;k++){
         ...
         if ((nmatch==ndaug)&&
	       ...
	       (thedecaymodel->getModelName()=="LUNDCHARM")||
