...
# Check if a folder exists. Exit the script if it doesn't.
{
	...
	if [ ! -d ${folderToCheck} ]; then
		echo -e "${gErrorColorCode}ERROR: Folder \"${folderToCheck}\" does not exist\e[0m"
		exit
	fi
}
...
# Check if a file exists. Exit the script if it doesn't.
{
	...
	if [ ! -s "${fileToCheck}" ]; then
		echo -e "${gErrorColorCode}ERROR: File \"${fileToCheck}\" does not exist\e[0m"
		exit
	fi
}
...
function ChangeWindowsLineEndingsToUnix()
...
# Feed this function a directory ($1) and it will list all files within that directory including their absolute paths. The output will be written to a file ($2). If you wish, you can only list files of a certain extension ($3).
{
	# * Import function arguments
